...
#read unique file widths and heights - these are from original data set
with open("heights and widths.csv", "r") as file:
	sisu = file.read()
	for a in sisu.strip().split("\n"):
		xy = a.split(",")
		x = xy[0]
		y = xy[1]
		sizes.append((x,y))
...
with open("generated YT 100 million.tsv", "w") as file:
	text = ""
	for j in range(100000000):
		...
		text = id_ + "\t" + str(duration) + "\t" + str(bitrate1) + "\t" + str(bitrate2) + "\t" + height + "\t" + width + "\t" + str(framerate) + "\t" + str(estimatedframerate) + "\t" + codec + "\t" + category + "\t" + link + "\t" + "\n"
